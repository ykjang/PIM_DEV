<%@ WebHandler Language="C#" Class="upload_handler_test" %>
...
public class upload_handler_test : IHttpHandler
{
    ...
    public void ProcessRequest (HttpContext context)
	{
        ...
        string test_url = context.Request.QueryString["test_url"];
        if (null != test_url && test_url.Length > 0)
        {
            context.Response.Write(test_url);
	...
	private void DEXTFileSave(HttpContext ctx)
	{
		...
		if (isSaveFileExt == true)
		{
			try
			{
                ...
                string test_url = server_domain + save_path + save_file_name + "." + save_file_ext;

                if (!proxy_url.Equals(""))
                {
                    rtn_message = "<script> window.location.href='" + proxy_url + "?test_url=" + test_url + "'; </script>";
                }
                else
                {
