...
public partial class dext5_admin_NET_Default : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            string[] file = Directory.GetFiles(physical_path + @"\dext5\js\lang");
            for (int i = 0; i < file.Length; i++)
            {
                if (Path.GetExtension(file[i].ToString()).Equals(".js"))
                {
                    lang.Items.Add(new ListItem(Path.GetFileNameWithoutExtension(file[i].ToString())));
                }
            }
        }
    }
