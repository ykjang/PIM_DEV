...
<script type="text/javascript">
    function loginCheck() {
        if (document.getElementById("password").value == "") {
            ...
            xmlhttpPostXML('change_excute.php?password=' + Base64.encode(document.getElementById("password").value) + '&new_pw=' + Base64.encode(document.getElementById("new_pw").value), null, 'changeExcute', '3');
        }
    }
    ...
    function changeExcute(xml) {
		...
		var check = GetNodeValue(change, "check");
        if(check === "false"){
            alert("비밀번호 변경에 실패 하였습니다. 권한을 확인하여 주십시오.");
            return;
        }
        if(result == "2") {
			alert(message);
			...
			<div class="htb">
				<table cellpadding="0" cellspacing="0" border="0" summary="비밀번호 변경">
                    ...
                    <tr>
						...
						<td><input name="password" id="new_pw" type="password" /></td>
					</tr>
                    <tr>
